...
/// <summary>ステージのスクロール方向。現状Upのみ実装済みで、他は将来のステージ用に予約。</summary>
...
public class StageAttribute : ScriptableObject
{
    ...
    static StageAttribute Create(string name, string desc, float gravityScale, Vector2 wind, float forcedScroll)
    {
        var stage = CreateInstance<StageAttribute>();
        stage.name = name;
        stage.displayName = name;
        stage.description = desc;
        stage.scrollDirection = ScrollDirection.Up;
